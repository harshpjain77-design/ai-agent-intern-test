...
## ⚙️ Explicit Technical Stack & Choices

In accordance with requirement #3, below is an explicit statement of the underlying technical choices:

- **Model Used**: Deterministic Python Intent Router + Rule-Gated Support Agent Engine (Local deterministic model pipeline).
- **Embedding Approach**: Sparse Term-Frequency Vector Representation combined with BM25 Cosine Scoring and Metadata Authority Adjustments (`+2.0` active official policies, `-4.0` superseded policies, `-100.0` internal notes).
- **Framework**: FastAPI (Async Web Interface & API) + Pydantic v2 (Strict Schema Validation Allowlists) + Pytest.
- **Storage Approach**: File-based JSON Order Data Store (`data/orders.json`) & Local Markdown File Vector Index (`knowledge-base/*.md`).
...
The system is configured to run deterministically out of the box without requiring external API tokens. Key environment variables are listed in `.env.example`:
...
git clone https://github.com/harshpjain77-design/ai-agent-intern-test.git
...
### 5. Running the Tests & Evaluation Suite
...
## 🤖 AI Coding Tools Disclosure & Flawed Suggestion Example

In accordance with requirement #9:

* **AI Coding Tools Used**: Gemini 3.6 Flash (Antigravity Agentic AI Assistant).
* **Usage**: Code auditing, architecture refactoring, unit test generation, and documentation refinement.
* **Concrete Example of Flawed AI Suggestion**: During initial construction of the `ResponseGuard` validation gate, an AI suggestion recommended applying the raw input privacy regex `PRIVATE = re.compile(r"...|\baddress\b|...")` directly to generated output strings. When tested against order cancellation queries, this caused false-positive refusals on valid support responses explaining "address correction" policies. The issue was resolved by replacing it with a targeted `PII_LEAK` pattern that validates actual customer PII structures (email formats, risk scores, internal warehouse notes) rather than general support terminology.
